        ...
        <div class="min-w-0 flex-1 flex flex-col gap-5">
            ...
            <div>
                <nav class="flex items-center gap-1.5 text-[11.5px] font-medium text-slate-400">
                    ...
                    <span class="text-slate-600">Règles des points</span>
                </nav>
                <h1 class="mt-1 text-[22px] font-black tracking-tight text-slate-900">Règles des points</h1>
            </div>
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Devise</p>
                    <p class="mt-1 text-[22px] font-black text-slate-900">{{ $settings->currency }}</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Points par montant</p>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Remise annuelle</p>
                    <p
                        class="mt-1 text-[22px] font-black {{ $settings->annual_reset ? 'text-emerald-600' : 'text-slate-400' }}">
                        {{ $settings->annual_reset ? 'Activée' : 'Désactivée' }}
                    </p>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="mb-5 text-[14px] font-bold text-slate-800">Modifier les paramètres</h2>
                <form id="points-form" class="space-y-5">
                    @csrf
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            <label class="mb-1.5 block text-[12px] font-semibold text-slate-600">Devise</label>
                        ...
                        <div>
                            <label class="mb-1.5 block text-[12px] font-semibold text-slate-600">Montant (unité)</label>
                            <input type="number" name="amount_value" value="{{ $settings->amount_value }}"
                                min="0.01" step="0.01"
                                class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-[13px] font-semibold text-slate-900 outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-[12px] font-semibold text-slate-600">Points accordés par
                                unité</label>
                        ...
                        <div class="flex items-end pb-0.5">
                            <label class="flex cursor-pointer items-center gap-3">
                                ...
                                <span class="text-[13px] font-semibold text-slate-700">Remise annuelle des points</span>
                            </label>
                        </div>
                    </div>
                    <p class="text-[12px] text-slate-400">
                        Formule actuelle&nbsp;: pour chaque
                        <strong>{{ number_format((float) $settings->amount_value, 2, ',', ' ') }}
                            {{ $settings->currency }}</strong> dépensé, le client gagne
                        <strong>{{ number_format((float) $settings->points_value, 2, ',', ' ') }} point(s)</strong>.
                    </p>
