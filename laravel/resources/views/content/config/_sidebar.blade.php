...
<aside class="w-72 shrink-0">
    <div class="sticky top-20 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-5 py-4">
            <p class="text-[11px] font-bold uppercase tracking-widest text-slate-400">{{ __('config.sidebar_title') }}
            </p>
        </div>
                ...
                <a href="{{ route($item['route']) }}"
                    ...
                    <span
                        ...
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
                            stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4">{!! $item['icon'] !!}</svg>
                    </span>
                    {{ $item['label'] }}
                </a>
            @endforeach
